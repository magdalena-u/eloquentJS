...
class Monster {
    ...
    static create(pos) {
        return new Monster(pos.plus(new Vec(0, 0.2)), 1);
    }

    update(time, state) {
        const moveMonster = this.pos.plus(new Vec(this.speed * time, 0));
        if (state.level.touches(moveMonster, this.size, 'wall'))
            return new Monster(moveMonster, -this.speed);
        return new Monster(moveMonster, this.speed);
    }

    collide(state) {
        const playerY = state.player.pos.y + state.player.size.y;
        const monsterY = this.pos.y + this.size.y;
        if (playerY < monsterY) {
            const index = state.actors.indexOf(this);
